    ...
    <div className="w-full max-w-2xl mx-auto my-6 p-8 rounded-2xl border border-emerald-400/40 bg-gradient-to-br from-emerald-950/60 to-black shadow-2xl shadow-emerald-900/20">
      <div className="text-center mb-6">
        <span className="inline-block px-3 py-1 mb-3 text-xs font-bold uppercase tracking-[0.2em] text-emerald-300 bg-emerald-500/10 border border-emerald-400/20 rounded-full">
          {advisor.preliminaryDemoProposal}
        </span>
        <h2 className="text-2xl font-black text-white">{advisor.executiveGrowthPlan}</h2>
      </div>

      <div className="space-y-6">
        <div className="p-4 rounded-lg bg-white/5 border border-white/10">
          <h4 className="text-sm font-semibold text-emerald-300 mb-2">{advisor.coreStrategy}</h4>
        ...
        <div className="p-4 rounded-lg bg-white/5 border border-white/10">
          <h4 className="text-sm font-semibold text-emerald-300 mb-2">{advisor.executionApproach}</h4>
          <p className="text-sm text-emerald-50/80 leading-relaxed">
          ...
          <>
            <div className="p-4 rounded-lg bg-white/5 border border-white/10">
              ...
              {plan.missingDependencies.filter(d => d.criticality === 'blocker').length > 0 && (
                <div className="mb-4">
                  <p className="text-xs font-bold text-red-400 mb-1 flex items-center gap-1"><AlertTriangle size={12}/> {advisor.blockingCriticalDependencies}:</p>
            ...
            <div className="p-4 rounded-lg bg-emerald-500/10 border border-emerald-400/30">
              <h4 className="text-sm font-semibold text-emerald-300 mb-2 flex items-center gap-2">
                <PlayCircle size={16} /> {advisor.nextImmediateAction}
              </h4>
              <p className="text-sm text-emerald-50/90 font-medium">
                {plan.nextRecommendedAction ? plan.nextRecommendedAction.title : advisor.reviewAndApproveExecutionPlan}
              </p>
              <p className="text-xs text-emerald-50/50 mt-1 capitalize">
                {advisor.recommendedPhase}: {plan.nextRecommendedAction ? plan.nextRecommendedAction.phase.replace('_', ' ') : advisor.preparation}
              </p>
            </div>
          </>
        )}
        ...
        {contentPlan && contentPlan.contentAssets.length > 0 && (
          <div className="p-4 rounded-lg bg-emerald-500/10 border border-emerald-400/30">
            <h4 className="text-sm font-semibold text-emerald-300 mb-2 flex items-center gap-2">
              <FileText size={16} /> {advisor.productionPlan}
            </h4>
            ...
            <ul className="space-y-2">
              {contentPlan.contentAssets.map(asset => (
                <li key={asset.id} className="text-xs text-emerald-50/80 p-2 rounded bg-white/5 border border-white/5 flex justify-between items-center">
                  ...
                  <span className="text-[10px] text-emerald-200/50 uppercase tracking-wider">{presentGrowthEnum('priority', asset.priority, advisor)}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        {contentBrief && (
          <div className={`p-4 rounded-lg border ${
            ...
            <h4 className={`text-sm font-semibold mb-2 flex items-center gap-2 ${
              ...
              <CheckCircle size={16} />
              {contentBrief.status === 'approved' ? advisor.briefApproved : advisor.briefProposedForReview}
            </h4>
            <p className="text-sm text-emerald-50/80">
              {advisor.asset}: <strong>{contentBrief.selectedAsset?.title || advisor.unassigned}</strong>
            </p>
          </div>
        )}
      </div>
